...
while True:
    ...
    if landmark_points:
        ...
        '''these numbers are the landmarks of our eyes.
        we can insert the array and loop through it'''
        for landmark in left_eye:
            x = int(landmark.x * frame_width)
            '''these lines are detecting the axis on our face'''
            y = int(landmark.y * frame_height)
            cv2.circle(frame , (x, y), 3,(0,255,255))
        'print(left_eye[0].y - left_eye[1].y)'
        if(left_eye[0].y - left_eye[1].y) < 0.0001:
            print('Clicked')
            '''if the values are above of 0.021... then eyes are open and
            if that values are below of 0.001 then eyes are closed '''
